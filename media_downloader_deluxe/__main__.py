...
try:
    ...
    import yt_dlp  # type: ignore[import-not-found]  # pyright: ignore[reportMissingImports]  # noqa
    import yt_dlp.version  # type: ignore[import-not-found]  # pyright: ignore[reportMissingImports]  # noqa
...
class Window(QMainWindow, Ui_MainWindow):
    ...
    def ask_update_ytdlp(self) -> None:
    ...
    def setupUi(self, *args: Any, **kwargs: Any) -> None:
        super().setupUi(*args, **kwargs)  # type: ignore[no-untyped-call]
    ...
    def start(self) -> None:
        ...
        if type == enums.Type.Music:
            quality: Union[
                enums.Quality, enums.MusicQuality
            ] = enums.MusicQuality(self.quality_box.currentIndex())
        else:
            quality = enums.Quality(self.quality_box.currentIndex())
        max_parallel = config.get_config_value("max_parallel_downloads")  # noqa

        def err_callback(url: str, err: Any = None) -> None:
            if self.manager:
                self.manager.killall()
            self.should_show_dl_error = url

        def thread_done_callback(
            url: str, success: Optional[bool] = None
        ) -> None:
            if not self.downloading:
                # Got cancelled
                return
            ...
            if not parallel:
                try:
                    ...
                    pass
            if self.manager and self.manager.is_completed():
    ...
    def update_progress(self) -> None:
        ...
        if self.manager:
            ...
            if self.manager.data.get("parallel"):
                ...
                if self.manager.current_thread:
                    self.progress_bar.setValue(
                        self.manager.current_thread.percent
                    )
            total = len(threads)
            finished = 0
            for thread in threads:
    ...
    def show_success(self, amount: int) -> None:
        utils.show_info(
            self,
    ...
    def show_download_error(self, url: str) -> None:
        utils.show_error(
            self,
    ...
    def change_lang(self, locale: str) -> None:
    ...
    def text_edit_changed(self) -> None:
    ...
    def closeEvent(self, event: Union[QCloseEvent, None]) -> None:
        if self.downloading:
    ...
    def licenses(self) -> None:
        dialog = LicensesDialog(self, self.lang)
        dialog.exec()

    def list_of_supported_sites(self) -> None:
        if platform.system() == "Windows":
            try:
                webbrowser.WindowsDefault().open(  # type: ignore[attr-defined]
...
class LicensesDialog(QDialog, Ui_Licenses):
    def __init__(self, parent: QMainWindow, lang: lang.LangDict) -> None:
    ...
    def setupUi(self, *args: Any, **kwargs: Any) -> None:
        super().setupUi(*args, **kwargs)  # type: ignore[no-untyped-call]
        self.setWindowTitle(self.lang["window_licenses"])
        self.header.setText(self.lang["licenses_header"])
...
class AboutDialog(QDialog, Ui_About):
    ...
    def setupUi(self, *args: Any, **kwargs: Any) -> None:
        super().setupUi(*args, **kwargs)  # type: ignore[no-untyped-call]
        self.setWindowTitle(self.lang["window_about"])
        self.label.setText(self.lang["about_author"])
        self.version.setText(self.lang["about_version"].format(
            version=__version__,
        ))
...
class SettingsDialog(QDialog, Ui_Settings):
    ...
    def setupUi(self, *args: Any, **kwargs: Any) -> None:
        super().setupUi(*args, **kwargs)  # type: ignore[no-untyped-call]
        self.setWindowTitle(self.lang["window_settings"])
        ...
        self.label.setText(self.lang["settings_header"])
        self.label_2.setText(
            self.lang["settings_max_parallel_downloads"]
        )
        self.output_path_label.setText(
            self.lang["settings_default_output_path"]
        )
        self.output_path_change_btn.setText(
            self.lang["settings_change"]
        )
        self.label_3.setText(self.lang["settings_ytdlp_version"])
        self.update_ytdlp.setText(self.lang["settings_update"])

    def update_ytdlp_action(self) -> None:
        ...
        try:
            if utils.is_ytdlp_latest_version():
                utils.show_info(
                    self,
                    self.lang["settings_update_uptodate_title"],
                    self.lang[
                ...
                self.update_ytdlp.setText(
                    self.lang["settings_update"]
                )
                utils.show_info(
                    self,
                    self.lang["settings_update_updating_title"],
                    self.lang["settings_update_updating_desc"].format(
                        old=old_version,
                        new=new_version,
                    ),
                )
            ...
            utils.show_error(
                self,
                self.lang["settings_update_noconn_title"],
                self.lang["settings_update_noconn_desc"],
            )
        self.update_ytdlp.setDisabled(False)

    def change_output_path(self) -> None:
        file_dialog = QFileDialog(self)
        file_dialog.setFileMode(QFileDialog.FileMode.Directory)
